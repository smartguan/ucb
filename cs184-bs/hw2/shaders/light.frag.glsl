...
uniform vec4 pointLightPosn[10] ; 
...
*/
	    ...
	    for(i=0; i<10; i++){
			...
			if(pointLightPosn[i].w == 0) direction = normalize(pointLightPosn[i].xyz);
			else {
				position = pointLightPosn[i].xyz / pointLightPosn[i].w ; 
				direction = normalize (position - mypos) ; // no attenuation 
			}
			halfdir = normalize (direction + eyedirn) ;  
			col += ComputeLight(direction, pointLightColor[i], normal, halfdir, diffuse, specular, shininess) ;
		}
...
*/        
        gl_FragColor = ambient + emission + col ; 
        }
}
